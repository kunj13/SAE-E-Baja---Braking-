...
max_speed_kmph = 45;
max_speed_mps = 12.5;
mu_tyretoroad = 0.65;               % + static cofficient 
mu_tyretoroad_kinetic = 0.65;        % kinetic cofficient
mu_pad=0.4;
h_cog= 0.533;
wheelbase= 1.315;
static_weight_ratio = 40/60;        % Front to Rear
wheel_radius_front=0.27305;
wheel_radius_rear=0.292;
pedal_ratio = 5;
pedal_leverage=0.9;
man_pedal_force = 140;
front_disc_radius = 160*0.001;
rear_disc_radius = 187*0.001;
dia_eff_front = (front_disc_radius- 20*0.001);
dia_eff_rear = (rear_disc_radius - 20*0.001);
caliper_piston_dia_front = 1*25.4*0.001;
caliper_piston_dia_rear = 1*25.4*0.001;

%master cylinder required values
    tmc_piston_dia = (19.05)*0.001;
...
%`average deceleration for whole stop
    ...
    x = max_speed_mps*g;
    y = ((max_speed_mps/mu_tyretoroad_kinetic) + 0.3*g);
...
%angualar acceleration of rotating tyre system
...
%brake torque calculations
    ...
    brake_torque_rear=((brake_force_rear_tyre)*wheel_radius_rear) + inertial_torque_suspension_rear + inertial_torque_transmission_total;


    force_tmc = pedal_ratio*pedal_leverage*man_pedal_force/2;
    ...
    caliper_force_rear = 4*brake_line_pressure*area_caliper_piston_rear;
...
%master cylinder losses( in centimeter cube)
    Volume_loss = brake_line_pressure*(10^(-6))*(10^(0.025*(tmc_piston_dia*1000) - 2.3)) ;
